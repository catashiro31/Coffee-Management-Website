...
const roast_select = ["Bọt lạnh","No Brown Sugar Cream Cold Foam",
    "Light Brown Sugar Cream Cold Foam", "Brown Sugar Cream Cold Foam",
    "Extra Brown Sugar Cream Cold Foam"
]
const topping_select = [
    "Toppings", "Extra Cinnamon Powder", "Light Cinnamon Powder", "No Cinnamon Powder", "Cinnamon Powder" 
]
const milk_select = [
...
let init_roast = 3;
let roast = 3;
let roastPrice = 2; // Giá mỗi shot, có thể điều chỉnh
let type_milk = [0,1, 2, 6, 7, 8];
let type_topping = [0, 5, 2, 6, 8];
let type_roast = [0, 4, 2, 6, 6];

function updatePrice() {
    // Tính giá: giá kích thước + (số shots * giá mỗi shot)
    if (selectedSize != null && selectedImageIndex != null) {
        cost = sizes[selectedImageIndex] + (roast * roastPrice) + type_milk[select1.selectedIndex] + type_topping[select2.selectedIndex] + type_roast[select3.selectedIndex];
    } else {
        cost = 0;
    }
...
function add_order() {
    if (selectedSize == null) {
        ...
        \nKích thước: ${selectedSize.toUpperCase()}, Sữa: ${milk_select[select1.selectedIndex]}, Topping: ${topping_select[select2.selectedIndex]}, Bọt lạnh: ${roast_select[select3.selectedIndex]}
        \nGiá: ${cost.toFixed(2)}$`);
    }
}
